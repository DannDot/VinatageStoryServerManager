...
void main() 
{
	...
	float murkiness=max(0.0, getUnderwaterMurkiness() - fogAmount);
	if (murkiness > 0.05) discard;

	vec4 texColor;
	
	float vn = max(0.0, 0.9 - abs(fragNormal.y));
	float wfc;
	...
	vec4 rgbaFinal = rgba;
#if FOAMEFFECT > 0
	rgbaFinal.a = rgba.a + max(0.0, texColor.a - 0.4);
#else
	...
	if (rgbaFinal.a > 0) {
		...
		if (isLava) {
			...
			float rgbAdd = max(0.0, bright*(diff * 0.3 + noise * 0.1));
			if (doLightFoam) {
				rgbAdd *= 0.5;
			}
			
			texColor.rgb += vec3(rgbAdd, rgbAdd, rgbAdd);
			texColor.a += (max(0.0, diff/16 + noise/(12 - 8*min(1,windSpeed))) + vn * 0.25) / clamp(fresnel, 0.5, 1);
			...
			if (skyExposed > 0) {
				...
				float upness = max(0.0, dot(fragNormal, vec3(0,1,0))); // Only do specular reflections on up faces
			...
			texColor.rgb *= 1 + f;
			texColor.a += max(0.0, 0.5 - texColor.a)*0.5*f; // Some extra alpha for droplet noise where there is low alpha
